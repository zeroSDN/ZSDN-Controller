...
app.controller('moduleCtrl', [ '$scope','$http', function(scope, http) {
	 ...
	 */
	function XHRUpdate() {
		http.get(url + '/modules',{cache: false}).
			then(function (response){
				...
				console.warn("Error while calling modules. Status Code: " + response.status + " " + response.statusText);
		});
	};
	...
	scope.changeState = function changeState(row) {
		if(row.currentState == "Active") {
			// disable
			http.put(url + '/modules/control?id='+ row.UniqueId + '&action=disable').
				then(function(response){
					XHRUpdate();
				}, function(response){
					// errorCallback
			...
			// enable
			http.put(url + '/modules/control?id='+ row.UniqueId + '&action=enable').
				then(function(response){
					XHRUpdate();
				}, function(response){
					// errorCallback
	...
	scope.stopModule = function stopModule(row) { 
		http.put(url + '/modules/control?id='+ row.UniqueId + '&action=stop').
			then(function(response){
				XHRUpdate();
			}, function(response){
				// errorCallback
